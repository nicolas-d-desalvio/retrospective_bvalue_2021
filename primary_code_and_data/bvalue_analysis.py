...
#-start_time 1825 -na_time 0.05 -precut_mag 1 -dist_thresh 3000 -grid 200
#-start_time -1 -na_time -1 -precut_mag -1 -dist_thresh -1 -grid 200
...
else: # To run within editor and not a command line, parameters can be set using lines 37-41
    start_t = 10*365 # 10 years (in days)
    na_time = 0.1 # 0.1 days
    precut_magnitude = 1.5 # M1.5
    dist_threshold = 3000 # 3km, enter in meters
    grid = 200
# Alert threshold must be manually set in the plotting function if desired, otherwise it is solely applied in post-processing
# End Times are set within the main function, they are in the 'for r in [end times]' loop
...
def distance_from_plane_cut(plane_vectors,parameters,Event_of_interest,total_data,times,fault_dimensions,dist_threshold,grid): #Takes the strike and dip vectors, and data (along with event of interest and the parameters dictionary)
    ...
    produce_rake_list = False # Creates a file with a list of rakes for the selected nodal plane. This is useful for creating subsets of different faulting types during post-processing applications, but the file only needs to be created once
    ...
    if len(total_data_first_hour[distance_mask_1_fh,3]) > (len(total_data_first_hour[distance_mask_2_fh,3])):
        ...
        if Event_of_interest == 'first':
            ...
            if produce_rake_list == True:
                rake_file = open('rake_file.txt','a')
                rake_file.write(str(parameters['FP_rake1_event1'])+'\n')
                rake_file.close()
        else:
            print('The Selected Strike is',parameters['FP_strike1_event2'])
            if produce_rake_list == True:
                rake_file = open('rake_file.txt','a')
                rake_file.write(str(parameters['FP_rake1_event2'])+'\n')
                rake_file.close()
        ...
        if Event_of_interest == 'first':
            ...
            if produce_rake_list == True:
                rake_file = open('rake_file.txt','a')
                rake_file.write(str(parameters['FP_rake2_event1'])+'\n')
                rake_file.close()
        else:
            print('The Selected Strike is',parameters['FP_strike2_event2'])
            if produce_rake_list == True:
                rake_file = open('rake_file.txt','a')
                rake_file.write(str(parameters['FP_rake2_event2'])+'\n')
                rake_file.close()
    else:
...
def pre_plot_pre_prep(pre_data_orig,parameters,mc_pre,pre_times_orig,total_data): #Takes the original pre data, times, and mc. As well as the parameters dictionary
    ...
    label_pre = 'Pre-Event: b = {:.02f}'.format(bv) + ' $ \pm $ ' + '{:.02f}'.format(sig1_pre)  
...
def pre_plot_bt_prep(bt_data_orig,parameters,mc_bt,bt_times_orig,Mrange,bt_times): # This does same as above function, but for bt.
    ...
    # Set up FMD Label for pre-events
    label_bt = 'Post-First Event: b = {:.02f}'.format(bv_bt) + ' $ \pm $ ' + '{:.02f}'.format(sig1_bt) 
...
def plotting(parameters,Mrange,Ncumpre,Tcatpre,Mrange_pre,N1,label_pre,Mrange_bt,N2,Ncumbt,Tcatbt,tdiff,Mrange_aft,N3,label_aft,times,breference,bt_times,pre_times,aft_times,bpre_ts,bbt_ts,baft_ts,sig1bt_ts,sig1pre_ts,sig1aft_ts,percent_change,percent_change2,percent_change_uncertainty,percent_change2_uncertainty,TLS1,TLS1_uncertainty,TLS2,TLS2_uncertainty,bv_bt,bv,label_bt,second_quake_exsists,Event_of_interest,i,path,Ncumaft):
    ...
    ax.set_yscale('log')
    ax.legend(fontsize=12,loc='upper center', bbox_to_anchor=(0.5, -0.1),fancybox=True, shadow=True)
    ax.set_title('FMD')
...
def main(na_time,dist_threshold,precut_magnitude,start_t,grid,path):
    ...
    make_plots = False # Do you want figures to be generated for every earthquake?
    ...
    for i in tqdm(range(len(main_events[:,0]))):
        ...
        if isinstance(i,int) == True:
            ...
            for r in [0.25,0.5,0.75,1,2,3,4,7,15,30,45,60,90,120,150,180,365,730,1095]: # End Times
...
# Code to name unnamed earthquakes from their location and generate the renamed_eq_locations_file
# However, actual named EQs must be manually overwritten
generate_location_name_file = False # Do you wish to generate this file?
if generate_location_name_file == True:
    ...
    for i in range(len(main_events)):
        ...
        try:
            ...
            result_summary[i,0] = 'Not Found'
            continue
        try:
            result_summary[i,0] = location.raw['address']['city']
        except KeyError:
            try:
                result_summary[i,0] = location.raw['address']['county']
            except KeyError:
                try:
                    result_summary[i,0] = location.raw['address']['state']
                except KeyError:
                    result_summary[i,0] = 'Not Found'
                    continue
