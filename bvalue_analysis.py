...
if dist_threshold < 0:
    dist_threshold = 'Auto'
#
# To specify Automatic, use 'Auto'
# Use the code below if not running from command line, comment out lines 23-43 above in this case.
# =============================================================================
# start_t = 10*365 # Start Time
# 
# na_time = 0.1 # No Alert Time
# 
# precut_magnitude = 1.5 # Precut Magnitude
# 
# #alert_threshold = 10 # Alert Threshold for Events Under M5
# 
# # Distance Threshold for Events Under M5
# dist_threshold = 2500 # ENTER IN METERS
# =============================================================================
...
def plotting(parameters,Mrange,Ncumpre,Tcatpre,Mrange_pre,N1,label_pre,Mrange_bt,N2,Ncumbt,Tcatbt,tdiff,Mrange_aft,N3,label_aft,times,breference,bt_times,pre_times,aft_times,bpre_ts,bbt_ts,baft_ts,sig1bt_ts,sig1pre_ts,sig1aft_ts,percent_change,percent_change2,percent_change_uncertainty,percent_change2_uncertainty,TLS1,TLS1_uncertainty,TLS2,TLS2_uncertainty,bv_bt,bv,label_bt,second_quake_exsists,Event_of_interest,i,path,Ncumaft):
    ...
    ax.scatter(Mrange,Ncumpre/Tcatpre,color = 'blue',marker = '.')
    ax.plot(Mrange_pre,N1,color = 'blue',label = label_pre)
    if bv_bt != -123456789:
        ax.scatter(Mrange,Ncumbt/Tcatbt,color = 'gray',marker = '.')
        ax.plot(Mrange_bt,N2,color = 'gray',label = label_bt)
    if second_quake_exsists == 'yes':
        ax.scatter(Mrange,Ncumaft/tdiff,color = 'red',marker = '.')
        ax.plot(Mrange_aft,N3,color = 'red',label = label_aft)
    ax.set_yscale('log')
    ax.legend(fontsize=12,loc='upper center', bbox_to_anchor=(0.5, -0.1),fancybox=True, shadow=True) #Legend position from https://stackoverflow.com/questions/4700614/how-to-put-the-legend-out-of-the-plot
    ax.set_title('FMD')
...
#     Uncomment to plot a pre-event b time series
#     try:
...
def main(na_time,dist_threshold,precut_magnitude,start_t,grid,path):
    ...
    for i in tqdm(range(len(main_events[:,0]))):
        ...
        if isinstance(i,int) == True:
            ...
            for r in [0.25,0.5,0.75,1,2,3,4,7,15,30,45,60,90,120,150,180,365,730,1095]:
                ...
                if second_quake_exsists == 'yes':
                    ...
                    TLS2 = np.nan
                    sig1_aft = np.nan
                print(Event_of_interest,second_quake_exsists)    
# =============================================================================
...
#                  plt.ylabel('Percent Change in b-value')
#                  plt.xlim(0,np.max(days_after)+10)    
#                  plt.savefig(path+'{:03d}'.format(i)+'_time_variability.png')
#                  #plt.savefig(path+'time_variability.svg')
#                  plt.show()
# =============================================================================
...
# For actual named EQs, these must be manually overwritten
